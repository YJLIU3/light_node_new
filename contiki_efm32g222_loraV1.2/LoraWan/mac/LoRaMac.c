 ...
 */
static uint8_t AckTimeoutRetries = 1;  //数据包重传次数

/*!
 * Number of trials to get a frame acknowledged
 */
static uint8_t AckTimeoutRetriesCounter = 1;//数据包重传计数器
...
#endif
                // RxDelay
//                LoRaMacParams.ReceiveDelay1 = ( LoRaMacRxPayload[12] & 0x0F ) ; //设置TX  RX 时间
//                
//               
//                
...
//                }
                LoRaMacParams.ReceiveDelay1 =8*CLOCK_CONF_SECOND;
                
                
                LoRaMacParams.ReceiveDelay2 =  CLOCK_CONF_SECOND*10;
                
                #if defined usart_enter  
                    printf("JOIN SET ReceiveDelay1:%d\n",LoRaMacParams.ReceiveDelay1);
                    printf("JOIN SET ReceiveDelay2:%d\n",LoRaMacParams.ReceiveDelay2);
                #endif 
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                else
                {
                    ...
                    #if defined usart_enter  
                        printf("confirmed Down Data Error\n");
                    #endif 
        ...
        if( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & MAC_TX_DELAYED ) == 0 ) )
        {
            ...
            if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
            {
                AckTimeoutRetriesCounter++;
                 ...
                 printf("AckTimeoutRetriesCounter:%d\n",AckTimeoutRetriesCounter);
...
static void OnRxWindow1TimerEvent( void )
{
    ...
    uint16_t symbTimeout = 100; // DR_2, DR_1, DR_0
    int8_t datarate = 0;
...
static bool SetNextChannel( TimerTime_t* time )
{
    ...
    uint8_t delayTx = 0;
    uint8_t enabledChannels[LORA_MAX_NB_CHANNELS];  //16
    TimerTime_t nextTxDelay = ( TimerTime_t )( -1 );//0XFFFFFFFF

    memset1( enabledChannels, 0, LORA_MAX_NB_CHANNELS );//初始化
...
#else
    if( CountBits( LoRaMacParams.ChannelsMask[0], 16 ) == 0 )//确保有信道
    {
...
//                    printf("%d\n",enabledChannels[nbEnabledChannels++] = i + j);
    ...
    {
        delayTx++;
        printf("TxDelayedTimer111: %d\n",AggregatedTimeOff);  
        nextTxDelay = AggregatedTimeOff - TimerGetElapsedTime( AggregatedLastTxDoneTime );
        printf("nextTxDelay: %d\n",nextTxDelay); 
        printf("AggregatedLastTxDoneTime: %d\n",TimerGetElapsedTime( AggregatedLastTxDoneTime )); 
    }
...
static LoRaMacStatus_t ScheduleTx( )
{
    ...
    CalculateBackOff( LastTxChannel );//计算发送后退时间  AggregatedTimeOff
...
static void CalculateBackOff( uint8_t channel )
{
    ...
    bool rndTimeOff = false;//随机数时间

    if( IsLoRaMacNetworkJoined == false )//查看是否已加入网络
    {
        joinDutyCycle = RetransmissionDutyCylce( );//重传占空比
    ...
    // Update Aggregated Time OFF  更新时间表
    //AggregatedTimeOff = AggregatedTimeOff + (uint32_t)((double)( TxTimeOnAir * AggregatedDCycle - TxTimeOnAir )/ ((double)1/CLOCK_CONF_SECOND));
    AggregatedTimeOff = AggregatedTimeOff +( TxTimeOnAir * AggregatedDCycle - TxTimeOnAir )* CLOCK_CONF_SECOND/1000;
    printf("TxTimeOnAir: %d\n",TxTimeOnAir); 
    printf("AggregatedTimeOff: %d\n",AggregatedTimeOff);  
...
static void ResetMacParameters( void )
{
    ...
    // Counters
    UpLinkCounter = 1;    //上行计数器
    DownLinkCounter = 0;  //下行计数器
    AdrAckCounter = 0;    //dar计数器

    ChannelsNbRepCounter = 1;     //上行数据重传次数

    AckTimeoutRetries = 1;        //应答超时请求
    AckTimeoutRetriesCounter = 1; //应答超时请求计数器
    AckTimeoutRetry = false;      //超时重传

    MaxDCycle = 0;    
    AggregatedDCycle = 10;       //占空比设置%0.1

    MacCommandsBufferIndex = 0;
    MacCommandsBufferToRepeatIndex = 0;

    IsRxWindowsEnabled = true;  //使能RxWindows

    LoRaMacParams.ChannelsTxPower = LoRaMacParamsDefaults.ChannelsTxPower;   //赋值默认发送功率
    LoRaMacParams.ChannelsDatarate = LoRaMacParamsDefaults.ChannelsDatarate; //赋值默认发送速率
    ...
    // Initialize channel index.  设置当前chanel:16
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    NodeAckRequested = false;//节点上行请求应答
    ...
    switch( macHdr->Bits.MType )
    {
        case FRAME_TYPE_JOIN_REQ:
            RxWindow1Delay = LoRaMacParams.JoinAcceptDelay1 - RADIO_WAKEUP_TIME;//  设置加网接收窗口时间
            ...
            LoRaMacDevNonce = Radio.Random( );//获取随机数
...
//                memcpy1( LoRaMacBuffer + pktHeaderLen, (uint8_t* ) payload, payloadSize );
            }
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    #if defined (usart_enter)  
        printf("send Frequency: %d\n",channel.Frequency);     
        printf("send datarate : %d\n",datarate);       
    #endif  
    ...
    { // Normal LoRa channel
        Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 0, datarate, 1, 8, false, true, 0, 0, false, CLOCK_CONF_SECOND*3 );
        TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );
    }
#elif defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
    Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
    ...
    #if defined (usart_enter)  
        printf("send data: ");
        for(uint8_t i=0;i<LoRaMacBufferPktLen;i++)
        {
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    LoRaMacDeviceClass = CLASS_A; //选择CLASS_A
    LoRaMacState = MAC_IDLE;      //MAC空闲

    JoinRequestTrials = 0;        //加网
    RepeaterSupport = false;

    // Reset duty cycle times
    AggregatedLastTxDoneTime = 0;  //32位
    AggregatedTimeOff = 0;          

    // Duty cycle
#if defined( USE_BAND_433 )
    DutyCycleOn = false; //占空比
#elif defined( USE_BAND_780 )
    DutyCycleOn = false;
#elif defined( USE_BAND_868 )
    DutyCycleOn = true;
    ...
    // Reset to defaults  配置默认参数
    LoRaMacParamsDefaults.ChannelsTxPower = LORAMAC_DEFAULT_TX_POWER; //发射功率初始化10db
    LoRaMacParamsDefaults.ChannelsDatarate = LORAMAC_DEFAULT_DATARATE;//速率初始化D0

    LoRaMacParamsDefaults.MaxRxWindow = MAX_RX_WINDOW;    //最大接收窗口3s
    LoRaMacParamsDefaults.ReceiveDelay1 = RECEIVE_DELAY1; //RECEIVE_DELAY1 1s
    LoRaMacParamsDefaults.ReceiveDelay2 = RECEIVE_DELAY2; //RECEIVE_DELAY1 2s
    LoRaMacParamsDefaults.JoinAcceptDelay1 = JOIN_ACCEPT_DELAY1; //JOIN_ACCEPT_DELAY1 5s
    LoRaMacParamsDefaults.JoinAcceptDelay2 = JOIN_ACCEPT_DELAY2; //JOIN_ACCEPT_DELAY2 6s

    LoRaMacParamsDefaults.ChannelsNbRep = 1;  //上行消息重复的数量  仅适用不需要应答的消息
    LoRaMacParamsDefaults.Rx1DrOffset = 0;   //设置上下行数据速率之间的偏移

    LoRaMacParamsDefaults.Rx2Channel = ( Rx2ChannelParams_t )RX_WND_2_CHANNEL;//接收窗口  频段434665000,  速率：DR_0 

    // Channel mask
#if defined( USE_BAND_433 )
    LoRaMacParamsDefaults.ChannelsMask[0] = LC( 1 ) + LC( 2 ) + LC( 3 );//使用前三个信道  信道屏蔽罩
#elif defined( USE_BAND_780 )
    ...
    ResetMacParameters( );//mac参数初始化

    // Initialize timers
    TimerInit( &MacStateCheckTimer, (funCallBack)OnMacStateCheckTimerEvent );
    TimerSetValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );

    TimerInit( &TxDelayedTimer, (funCallBack)OnTxDelayedTimerEvent );
    TimerInit( &RxWindowTimer1, (funCallBack)OnRxWindow1TimerEvent );
    TimerInit( &RxWindowTimer2, (funCallBack)OnRxWindow2TimerEvent );
    TimerInit( &AckTimeoutTimer, (funCallBack)OnAckTimeoutTimerEvent );
    ...
    PublicNetwork = true;
    SetPublicNetwork( PublicNetwork );//容许加入公共网络
    Radio.Sleep( );//睡眠
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    memset1( ( uint8_t* ) &MlmeConfirm, 0, sizeof( MlmeConfirm ) );//缓存区初始化
    ...
    switch( mlmeRequest->Type )
    {
        case MLME_JOIN:
        {
            ...
            MlmeConfirm.MlmeRequest = mlmeRequest->Type;//记录当前mac管理层命令

            if( ( mlmeRequest->Req.Join.DevEui == NULL ) ||
                ( mlmeRequest->Req.Join.AppEui == NULL ) ||
                ( mlmeRequest->Req.Join.AppKey == NULL ) )
            {
                return LORAMAC_STATUS_PARAMETER_INVALID;//返回参数无效
            }
            ...
            macHdr.Value = 0;
            macHdr.Bits.MType  = FRAME_TYPE_JOIN_REQ;//mac MTYPE

            ResetMacParameters( );//恢复上电默认设置

            JoinRequestTrials++;  //加网尝试 自动匹配速率
