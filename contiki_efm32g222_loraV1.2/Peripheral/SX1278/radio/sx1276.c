...
void SX1276Init( RadioEvents_t *events )
{
    ...
    TimerInit( &TxTimeoutTimer,    (funCallBack)SX1276OnTimeoutIrq );
    TimerInit( &RxTimeoutTimer,    (funCallBack)SX1276OnTimeoutIrq );
    TimerInit( &RxTimeoutSyncWord, (funCallBack)SX1276OnTimeoutIrq );
    
    SX1276Reset( );   //sx1278复位

    RxChainCalibration( ); //仅在复位后进行，所有寄存器复位

    SX1276SetOpMode( RF_OPMODE_SLEEP ); //设置成休眠模式

    for( i = 0; i < sizeof( RadioRegsInit ) / sizeof( RadioRegisters_t ); i++ )//寄存器初始化
    {
        SX1276SetModem( RadioRegsInit[i].Modem );
        SX1276Write( RadioRegsInit[i].Addr, RadioRegsInit[i].Value );
    }

    SX1276SetModem( MODEM_FSK );//设置为fsk模式
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    SX1276SetModem( modem );//设置模式

    paConfig = SX1276Read( REG_PACONFIG );//读取输出功率
    paDac = SX1276Read( REG_PADAC );//读取最大功率
...
{
    //3  assert_param( ( SX1276.Spi.Spi.Instance != NULL ) );//清空模式
    ...
    case MODEM_LORA:
        ...
        SX1276Write( REG_DIOMAPPING1, 0x00 );//IO口中断配置
